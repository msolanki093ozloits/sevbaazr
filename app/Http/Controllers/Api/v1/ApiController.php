...
class ApiController extends Controller
{
    ...
    public function Login(Request $request)
    {
        ...
        $user = User::where(['Phone' => $request->Phone, 'otp' => $request
            ->otp])
            ->first();
        if ($user === null)
        {
        ...
        else
        {
            ...
            $product = User::where(['Phone' => $request
                ->Phone])
                ->first();
            return response()
                ->json(['Status' => '1', 'data' => $product]);
    ...
    {
        $product = Brands::where('Brand_status', '=', 1)->orderBy('Brand_lavel', 'asc')
            ->get();
        return response()
            ->json(['Status' => '1', 'data' => $product]);
    ...
    public function Socialregister(Request $request)
    {
        ...
        if ($user === null)
        {
            ...
            $product->save();
            return response()
                ->json(['Status' => '1', 'message' => 'Successful', 'data' => $data]);
        }
        else
        {
    ...
    {
        $product = Brands::where('Brand_city', '=', $request->region_id)
            ->get();
        if ($product === null)
        {
    ...
    {
        if ($product = User::where(['id' => $request
            ->user_id])
            ->exists())
        {
            $product = User::where(['id' => $request
                ->user_id])
                ->first();
            return response()
                ->json(['Status' => '1', 'data' => $product]);
    ...
    {
        $product = Product::where(['Brand_name' => $request->Brand_id, 'product_status' => 1])
        ...
        else
        {
            ...
            return response()
                ->json(['Status' => '1', 'data' => $newarry1]);
        }
    }

    public function ProductAll(Request $request)
    {
        $product = Product::where(['product_status' => 1])->orderBy('product_id', 'desc')
            ->get();
        if (count($product) == 0)
        {
            return response()->json(['Status' => '0', 'message' => 'No Product Found']);
    ...
    {
        $product = Product::where(['product_status' => 1])->orderBy('product_rating', 'desc')
            ->get();
        if (count($product) == 0)
        {
            return response()->json(['Status' => '0', 'message' => 'No Product Found']);
    ...
    {
        $product = Brands::orderBy('Brand_lavel', 'asc')->take(6)
            ->get();
        $newarry = array();
        foreach ($product as $itm)
        {
    ...
    {
        $user = Carts::where('cart_id', '=', $request->cart_id)
            ->first();
        $productdetails = Product::where('product_id', '=', $request->product_id)
            ->first();
        $coupon = Coupon::where(['coupon_code' => $request->couponcode, 'product_id' => $request
            ->product_id])
            ->first();
        if (!empty($coupon))
        {
            if (strtotime(date('Y-m-d')) >= strtotime($coupon['start_date']) && strtotime(date('Y-m-d')) <= strtotime($coupon['end_date']))
            {
                ...
                Carts::where(['cart_id' => $request
                    ->cart_id])
                    ->update(['apply_code' => $request->couponcode, 'coupon_price' => $disp, 'price' => $price1, 'dis_price' => $price1 - $price1]);
                return response()->json(['Status' => '1', 'message' => 'Successful']);
            }
            else
            {
                return response()
                    ->json(['Status' => '0', 'message' => 'Coupon expired']);
            }
        }
        else
        {
    ...
    {
        $product = Carts::select('order_id')->orderBy('order_id', 'DESC')
    ...
    {
        $product = Product::orWhere('product_title', 'like', '%' . $request->name . '%')
            ->get();
        if (count($product) == 0)
        {
            return response()->json(['Status' => '0', 'message' => 'No Product Found']);
    ...
    {
        $product = Banner::where('region_id', '=', $request->region_id)
            ->get();
        if (count($product) > 0)
        {
            $newarry = array();
    ...
    public function CategoryLatestOffer(Request $request)
    {
        ...
        {
            $product = Product::where(['product_catgory' => $request->category_id, 'product_Offer_status' => 1])
                ->orderBy('product_offerpercent', 'desc')
                ->take(10)
                ->get();
        }
        else
        {
            $product = Product::where(['product_Offer_status' => 1])->orderBy('product_offerpercent', 'desc')
                ->take(10)
                ->get();
        }
        if (count($product) == 0)
    ...
    {
        $product1 = Product::where('product_id', '=', $request->product_id)
            ->first();
        $product = Product::where('product_catgory', '=', $product1['product_catgory'])->orderBy('product_id', 'desc')
            ->take(10)
            ->get();
        if (count($product) == 0)
        {
    ...
    public function ProductCategory(Request $request)
    {
        if ($request->category_id != 0)
        {
            $product = Product::where(['product_catgory' => $request->category_id, 'Brand_name' => $request->Brand_id, 'product_status' => 1])
                ->get();
        }
        else
        {
    ...
    public function ProductDesc(Request $request)
    {
        ...
        else
        {
            ...
            $productfav = Favourite::where(['product_id' => $product['product_id'], 'user_id' => $request
                ->user_id])
                ->get();
            ...
            $productcarts = Carts::where(['product_id' => $product['product_id'], 'user_id' => $request->user_id, 'order_id' => 0])
                ->first();
    ...
    {
        $productbrand = Brands::inRandomOrder()->take(4)
            ->get();
        $alldatadetals = array();
        ...
        foreach ($productdetasil as $itm1)
        {
            ...
            $productcarts = Carts::where(['product_id' => $itm1['product_id'], 'user_id' => $request->user_id, 'order_id' => 0])
                ->first();
            if (empty($productcarts))
            {
                $cart_status = 0;
        ...
        $product = Product::where(['product_status' => 1])->skip($dty)->take(4)
            ->get();
        $newarry = array();
        foreach ($product as $itm)
        {
            ...
            {
                $productfav = Favourite::where(['product_id' => $itm['product_id'], 'user_id' => $request
                    ->user_id])
                    ->get();
                //echo count($productfav);
                if (count($productfav) == 0)
            ...
            $productcarts = Carts::where(['product_id' => $itm['product_id'], 'user_id' => $request->user_id, 'order_id' => 0])
                ->first();
            if (empty($productcarts))
            {
                $cart_status = 0;
    ...
    {
        $product = Product::where(['product_Offer_status' => 1, 'product_status' => 1])->orderBy('product_offerpercent', 'desc')
            ->take(10)
            ->get();
        if (count($product) == 0)
        {
    ...
    {
        $user = Carts::where(['user_id' => $request->user_id, 'payment_order' => 0, 'order_id' => 0])
            ->get();
        if (count($user) > 0)
        {
    ...
    public function AddToCart(Request $request)
    {
        ...
        $user = Carts::where(['product_id' => $request->product_id, 'product_weight' => $request->weight, 'user_id' => $request->user_id, 'payment_order' => 0, 'order_id' => 0])
            ->first();
        if (Carts::where(['product_id' => $request->product_id, 'product_weight' => $request->weight, 'user_id' => $request->user_id, 'payment_order' => 0, 'order_id' => 0])
            ->exists())
        {
            return response()
                ->json(['Status' => '0', 'message' => 'Product already exists']);
        }
        else
        {
            $productdetails = Product::where('product_id', '=', $request->product_id)
                ->first();
            $unitdet = Unit::where('unit_id', '=', $request->Unit)
                ->First();
            ...
            $product->save();
            $user1 = Carts::where(['user_id' => $request->user_id, 'payment_order' => 0, 'order_id' => 0])
                ->get();
            return response()
                ->json(['Status' => '1', 'message' => 'Successful', 'data' => count($user1) ]);
        }
    }
    ...
    {
        $user = Carts::where(['order_id' => 0, 'user_id' => $request
            ->user_id])
            ->get();
        if (count($user) > 0)
        {
    ...
    {
        $rate = new \RocketShipIt\RocketShipIt\Rate('UPS');
    ...
    public function AddAddress(Request $request)
    {
        ...
        $product->save();
        return response()
            ->json(['Status' => '1', 'message' => 'Successful']);
    }
    public function userAddress(Request $request)
    {

        $product = Useraddress::where('user_id', '=', $request->user_id)
            ->orderBy('set_def', 'ASC')
            ->get();
        if (count($product) > 0)
        {
    ...
    {
        $product = Favourite::where(['user_id' => $request
            ->user_id])
            ->get();
        if (count($product) == 0)
        {
    ...
    public function RemoveNotification(Request $request)
    {
        ...
        $similproduct = Notification::where(['notification_id' => $request
            ->notification_id])
            ->first();
        return response()
            ->json(['Status' => '1', 'message' => 'Remove Successful']);

    }

    public function Favourites(Request $request)
    {
        $similproduct = Favourite::where(['product_id' => $request->product_id, 'user_id' => $request
            ->user_id])
            ->first();
        if (empty($similproduct))
        {
            ...
            $product->save();
            return response()
                ->json(['Status' => '1', 'favStatus' => '1', 'message' => 'Successful']);
        }
        else
        {
            Favourite::where(['product_id' => $request->product_id, 'user_id' => $request
                ->user_id])
                ->delete();
            return response()
                ->json(['Status' => '1', 'favStatus' => '0', 'message' => 'Remove Successful']);
        }
    }
    ...
    public function UpdateCart(Request $request)
    {
        ...
        else
        {
            ...
            Carts::where(['cart_id' => $request
                ->cart_id])
                ->update(['qty' => $request->qty, 'total_amt' => $productprice, 'price' => $price1, 'total_weight' => $weigh]);
    ...
    {
        $similproduct = Useraddress::where(['address_id' => $request
            ->address_id])
            ->first();
        if (empty($similproduct))
        {
        ...
        else
        {
            Useraddress::where(['address_id' => $request
                ...
                ->delete();
            return response()
                ->json(['Status' => '1', 'message' => 'Delete Successful']);
        }
    }

    public function Changeaddress(Request $request)
    {
        $vender_id = Carts::select('vender_id')->distinct()
            ->where(['user_id' => $request->user_id, 'payment_order' => 0, 'order_id' => 0])
            ->get();
        ...
        else
        {
            foreach ($vender_id as $itmven_id)
            {
                ...
                Useraddress::where(['address_id' => $request
                    ->address_id])
                    ->update(['set_def' => 0]);
                $vendtl = Carts::where(['vender_id' => $itmven_id['vender_id'], 'user_id' => $request->user_id, 'payment_order' => 0, 'order_id' => 0])
                    ->get();
                $unitsu = 0;
    ...
    public function sampleOrder($order_info)
    {
        ...
        foreach ($order_info->product as $key => $cart)
        {
            ...
            $order_items[] = ['name' => $cart
                ->product->product_title, 'sku' => 'chakra12' . $key, // For Discuss
            'units' => $cart->qty, // For Discuss
            'selling_price' => $cart
                ->product->price, 'discount' => $cart
                ->product->dis_price, 'tax' => '', 'hsn' => '', ];
    ...
    public function OrderDetails(Request $request)
    {
        ...
        $data = Order::where('ord_id', $request->get('order_id'))
            ->select(['ord_id', 'address_id', 'delivery_charge', 'totalamount', 'grandttl', 'gst_charge', \DB::raw('order_status as my_order_status') ])->with(['product' => function ($q)
        {
            $q->select(['cart_id', 'order_id', 'product_id', 'Unit', 'price', 'dis_price', 'qty', 'product_weight', 'total_amt'])
